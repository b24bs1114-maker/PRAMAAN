 ...
 * implemented with CSS keyframes plus the small amount of state that genuinely
 * needs JavaScript -- exit transitions.
 *
 * Rules this layer enforces:
 *  - Durations come from the token scale (150-250ms for interface movement),
 *    which `prefers-reduced-motion` collapses in CSS. Nothing here needs to
 *    branch on the media query except where a timer is involved.
